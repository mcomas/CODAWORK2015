...
for(i in 1:mc$G){
  ...
  p = p + geom_line(data=df.d.dens %>% mutate(f = func(x)), aes(x=x, y=f), size = 1, col = i, alpha=0.6)
}
...
for(i in hp[[7]]){
  ...
  p = p + geom_line(data=df.d.dens %>% mutate(f = func(x)), aes(x=x, y=f), size = 1, col = i[[1]], alpha=0.6)
}
