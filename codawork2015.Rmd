...
p + theme_bw() + 
  xlab(expression(tau[1]:tau[2])) + ylab('Shannon entropy') + coord_cartesian(xlim=c(0,1))
...
p + theme_bw() + 
  xlab(expression(tau[1]:tau[2])) + ylab('Aitchison distance') + coord_cartesian(xlim=c(0,1), ylim=c(-5, 1))
```
...
```{r, fig.width=4, fig.height=6, fig.show='hold'}
...
ggplot() + geom_line(data = d, aes(x = x, y = demp, col=other)) + theme_bw() + 
  xlab(expression(tau[2]:tau[1])) + ylab('DEMP criteria') + coord_cartesian(xlim=c(0,1)) + facet_grid(other~.)
...
ggplot() + geom_line(data = data.frame(x = seq(0.01, 1-other-0.01, 0.01)) %>% mutate(log = log((1-other-x) / x)), aes(x = x, y = log)) + theme_bw() + 
  xlab(expression(tau[2]:tau[1])) + ylab('Logratio') + coord_cartesian(xlim=c(0,1))
```
