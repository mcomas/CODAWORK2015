...
\begin{itemize}
\item Given the posterior probability vector $\left( \tau_{i1}, \tau_{i2}, \tau_{i3} \right)$.
\item In this case, the misclassification is given by $\mathbbm{1}\left( \forall j\; \tau_{i 2} \geq \tau_{i 1} \right)$
...
\item It measures how large is $\tau_{i I_b}$ with respect to $\tau_{i I_a}$.
...
\begin{itemize}
\item Given the posterior probability vector $\left( \tau_{i1}, \tau_{i2}, \tau_{i3} \right)$.
\item Now the misclassification is given by $log\left(\tau_{i 2}/\tau_{i 1} \right)$
\item For $\tau_3 = 0.1$\uncover<2->{, {\color{red}$\tau_3 = 0.3$}}\uncover<3->{ and {\color{green}$\tau_3 = 0.5$}.}
\end{itemize}
\begin{columns}
\column{0.7\textwidth}
...
If we knew that some subcompositions are forming a single cluster, then we could find a hierarchy inside each subcomposition independently without considering other subcompositions.
