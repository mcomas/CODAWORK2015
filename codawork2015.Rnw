...
\only<2->{ \[ f(x) = {\color{red}\pi_1 f_1(x;\theta_1)} + {\color{green}\pi_2 f_2(x;\theta_2)} + {\color{blue}\pi_3 f_3(x;\theta_3)}. \] }%
\item<3> Observation are assigned to the component most likely belong to.
\end{itemize}
...
\only<1-2>{\[ f(x) = {\color{red}\pi_1 f_1(x;\theta_1)} + {\color{green}\pi_2 f_2(x;\theta_2)} + {\color{blue}\pi_3 f_3(x;\theta_3)},\]}
\only<3->{\[ f(x) = {\color{red}\pi_1 f_1(x;\theta_1)} + {\color{blue} (\pi_2+\pi_3)\left\{ \frac{\pi_2}{\pi_2+\pi_3} f_2(x;\theta_2) + \frac{\pi_3}{\pi_2+\pi_3} f_3(x;\theta_3) \right\}},\]}
...
\only<2>{ \[ \left({\color{red}\pi_1 f_1(x_i;\theta_1)}, {\color{green}\pi_2 f_2(x_i;\theta_2)}, {\color{blue}\pi_3 f_3(x_i;\theta_3)} \right) \] }
\only<3->{ \[ \left({\color{red}\pi_1 f_1(x_i;\theta_1)}, {\color{blue}\pi_2 f_2(x_i;\theta_2) + \pi_3 f_3(x_i;\theta_3)} \right) \] }
\only<4>{\alert{The underlying mixture in both cases is the same.}}
\end{itemize}
...
Merge the components $I_a$ and $I_b$ that minimize the Shannon entropy.
\end{alertblock}
...
\]
\item To minimize the entropy after merging components $I_a$ and $I_b$, is equivalent to maximize
\[
...
pentropy = p + theme_classic() + 
  xlab(expression(tau[2])) + ylab('Shannon entropy increment') + coord_cartesian(xlim=c(0,1)) + theme(panel.grid=element_blank())
...
\[
\uncover<2>{\alert{-}}\sum_{i=1}^n {\|(\tau_{i I_a}, \tau_{i I_b})\|_\mathcal{A}}^2
\]
is \only<1>{minimum}\only<2>{\st{minimum} \alert{maximum}}.
\end{alertblock}
...
\begin{frame}[t]
\frametitle{The misclassification approach}

\begin{alertblock}{The misclassification approach (Hennig, 2010)}
Merge the components $I_a$ and $I_b$ that maximize the probability of assigning to component $I_b$ observations generated by $I_a$ (misclassification).
\end{alertblock}
...
\begin{itemize}
\item Given the posterior probability vector $\left( \tau_{i1}, \tau_{i2}, \tau_{i3} \right)$.
\item Remember, the misclassification is given by $\mathbbm{1}\left( \forall j\; \tau_{i I_{b}} \geq \tau_{i I_j} \right)$
...
Merge that components $I_a$ and $I_b$ that
\[
\frac{ \sum_{i=1}^n {\tau_{i I_a} {\color{red}log(\tau_{i I_b}/\tau_{i I_a})} }}{\sum_{i=1}^n \tau_{i I_a} }
\]
is minimum.
\end{alertblock}

\begin{itemize}
\item We've replaced the misclassification measure $\mathbbm{1}\left( \forall j\; \tau_{i I_{b}} \geq \tau_{i I_j} \right)$ by $log(\tau_{i I_b}/\tau_{i I_a})$.
\item It measures how large is $\tau_{i I_b}$ with respect to $\tau_{i I_b}$.
...
\begin{itemize}
\item Given the posterior probability vector $\left( \tau_{i1}, \tau_{i2}, \tau_{i3} \right)$.
\item Remember, now the misclassification is given by $log\left(\tau_{i I_{b}}/\tau_{i I_{a}} \right)$
\item For $\tau_3 = 0.1$\uncover<2->{, {\color{red}$\tau_3 = 0.3$}}\uncover<3->{ and {\color{green}$\tau_3 = 0.5$}.}
\end{itemize}
\begin{columns}
\column{0.7\textwidth}
...
pdemp<-p + theme_classic() + 
  xlab(expression(tau[2])) + ylab('Log-ratio criteria') + coord_cartesian(xlim=c(0,1))  + 
  theme(panel.grid=element_blank(),
        strip.text.y = element_text(size=12, face="bold"),
...
\begin{columns}
\frametitle{The advantages of compositional approaches}
...
d <- lapply(steps <- c(0.1, 0.3, 0.5), function(other) data.frame(x = seq(0, 1-other, 0.001)) %>% 
                                                            mutate(demp = ifelse(1-other-x < x & other < x, 1, 0),
                                                                   other = as.character(other))) %>% bind_rows %>% mutate(
                                                                     other = factor(other, labels = sprintf("%3.1f",steps)))

p = ggplot() + geom_point(data = d, aes(x = x, y = demp, col=other), shape=15, size=1) + facet_grid(other~.)
...
p + theme_classic() + 
  ...
  theme(panel.grid=element_blank(),
        ...
        strip.text.y = element_text(size=12, face="bold"),
        strip.background = element_rect(colour="black", fill="white"))+ylim(0,1) +
   scale_colour_manual(values=c(1,2,3))
@
...
\begin{itemize}
\item Matrix of posterior probabilities is not restricted to contain probabilities, they can be positive measurements containing relative information between components. 
...
If we knew that some subcompositions were going to be forming a single cluster, then we could find a parallel hierchical merging sequence inside each subcomposition in parallel without considering the other subcompositions.
...
\begin{itemize}
\item The introduced log-ratio approaches only deal with the probabilities of the components to be merged.
\item CoDa approach allows us to merge mixture components using well established principles
\end{itemize}

\begin{itemize}
\item The merging approach based on the amalgamation of probabilities (one observation can only be considered from one component). Can we use other forms of merging? Maybe geometrical mean between components? What does this form of merging means?
\end{itemize}
\end{frame}
...
\begin{frame}[plain]
  ...
  \bigskip
    \bigskip
      \bigskip
{\small You can get material related with this presentation at \\ http://mcomas.net/CODAWORK2015/}
