...
\only<2-4>{ \[ \left({\color{red}\pi_1 f_1(x_i;\theta_1)}, {\color{green}\pi_2 f_2(x_i;\theta_2)}, {\color{blue}\pi_3 f_3(x_i;\theta_3)} \right) \] }
\only<5>{ \[ \left({\color{red}\pi_1 f_1(x_i;\theta_1)}, {\color{blue}\pi_2 f_2(x_i;\theta_2) + \pi_3 f_3(x_i;\theta_3)} \right) \] }
\item<3-> A different approach is to consider a \alert{mixture of Gaussian mixtures}
\end{itemize}
...
\frametitle{The Shannon entropy approach}
\begin{itemize}
\item Given the posterior probability vector $\left( \tau_{i1}, \tau_{i2}, \dots \tau_{ik} \right)$.
\item To study the relation between $\tau_{i1}$ and $\tau_{i2}$, we can restrict to $\left( \tau_{i1}, \tau_{i2}, 1-\tau_{i1}-\tau_{i2} \right)$
\end{itemize}
...
\begin{itemize}
\item It only depends on component $I_a$ and $I_b$.
\end{itemize}
...
\only<1>{
<<fig.width=6.5, fig.height=5>>=
d <- lapply(steps <- c(0.1, 0.25, 0.5), function(other) data.frame(x = seq(0, 1-other, 0.001)) %>% 
